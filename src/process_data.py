...
def make_data(args):
    ## determine splits
    if args.task_name == 'R2T8':
        train_splits = []
        val_splits = ['R2R_val_unseen', 'R4R_val_unseen', 'R6R_val_unseen', 'R8R_val_unseen']
    elif args.use_test:
        train_splits = []
        val_splits = ['test']
    else:
    ...
    train_data = load_task_datasets(train_splits, args.task_name, postfix=args.split_postfix,
                                    tokenizer=tok, one_by_one_mode=args.one_by_one_mode)
    val_data = load_task_datasets(val_splits, args.task_name, postfix=args.split_postfix,
                                  tokenizer=tok, one_by_one_mode=args.one_by_one_mode)
    
    ### split for training
    if len(train_data) > 0:
        assert len(train_data['train']) >= args.batch_size, "data not enough for one batch, reduce the batch size"
    if args.curriculum_rl:
        if args.one_by_one_mode == 'period':
            il_train_data, _ = period_split(train_data, tok, 0, args.history)
            train_data = period_split_datasets(train_data, tok, args.history)
        elif args.one_by_one_mode == 'landmark':
            il_train_data, _ = landmark_split(train_data, tok, 0, args.history)
            train_data = landmark_split_datasets(train_data, tok, args.history)
        else:
            print("Error! Not implemented one by one model!")
            exit(0)
        ...
        if args.il_mode == 'period_split':
            ...
            print("Error! Training mode not available.")
            exit(0)
    ...
    if args.curriculum_rl:
        il_train_data = merge_data(il_train_data)
        train_data = {"rl": train_data, "il": il_train_data}
    
    ### make single data splitted sentence by sentence for evaluation
    if args.one_by_one:
        if args.one_by_one_mode == 'period':
            val_data = period_split_datasets(val_data, tok, args.history, use_test=args.use_test)
        elif args.one_by_one_mode == 'landmark':
            val_data = landmark_split_datasets(val_data, tok, args.history, use_test=args.use_test)
        else:
...
def load_task_datasets(splits, task, postfix='', tokenizer=None, one_by_one_mode=None):
    dataset = {}
    ...
    for split in splits:
        ...
        for item in load_dataset(split, task, postfix):
            if one_by_one_mode == "landmark":
                new_item = dict(item)
                new_item['instr_id'] = '%s_%d' % (item['path_id'], id_list[item['path_id']])
...
def add_history_to_data(data, history_heading, history_path, history_instr, history_instr_encoding):
    ...
    for tag, data in datasets.items():
        ...
        for d in data:
            ...
            for i, ins_sp in enumerate(ins_splited):
                ...
                new_d['instr_encoding'], new_d['instr_length'] = tok.encode_sentence(ins_sp + '.')
                new_d['heading'] = d['headings'][last_path_split_point - 1]
                new_d['remain_split'] = len(ins_splited) - i
                if history:
                    add_history_to_data(new_d, history_heading, history_path, history_instr, history_instr_encoding)
    ...
    for tag, data in datasets.items():
        ...
        for d in data:
            ...
            for i, ins in enumerate(d['instructions']):
                ...
                new_d['instr_encoding'], new_d['instr_length'] = tok.encode_sentence(ins)
...
def period_split_datasets(datasets, tok, history, use_test=False):
    splited = {}
    for tag, data in datasets.items():
        new_data = period_split_batch([data], tok, history, use_test)[0]
        splited[tag] = sorted(new_data, key=lambda item: len(item))
    return splited


def period_split_batch(batch, tok, history, use_test):
    new_batch = []
    for mini_batch in batch:
        ...
        for b in mini_batch:
            ...
            for i, ins_sp in enumerate(ins_splited):
                ...
                if ratio > 1:
                    ratio = 1
                path_split_point = math.ceil(len(b['path']) * ratio)
                
                new_d = copy.deepcopy(b)
                new_d['instructions'] = ins_sp + '.'
                new_d['instr_encoding'], new_d['instr_length'] = tok.encode_sentence(ins_sp + '.')
                if use_test:
                    new_d['path'] = b['path'] * 2
                    new_d['heading'] = float(b['heading'])
                else:
                    new_d['path'] = b['path'][last_path_split_point - 1:path_split_point]
                    new_d['heading'] = b['headings'][last_path_split_point - 1]
                if history:
                    add_history_to_data(new_d, history_heading, history_path, history_instr, history_instr_encoding)
                
                last_path_split_point = path_split_point
                new_d_list.append(new_d)
            assert new_d['path'][-1] == b['path'][-1]
            new_mini_batch.append(new_d_list)
        new_batch.append(new_mini_batch)
    return new_batch


def landmark_split_datasets(datasets, tok, history, use_test=False):
    splited = {}
    for tag, data in datasets.items():
        new_data = landmark_split_batch([data], tok, history, use_test)[0]
        splited[tag] = sorted(new_data, key=lambda item: len(item))
    return splited


def landmark_split_batch(batch, tok, history, use_test):
    new_batch = []
    for mini_batch in batch:
        new_mini_batch = []
        for b in mini_batch:
            ...
            for i, ins in enumerate(b['instructions']):
                ...
                if use_test:
                    ...
                    new_d['path'] = b['path'][i]
                    new_d['heading'] = float(b['headings'][i])
                if history:
                    add_history_to_data(new_d, history_heading, history_path, history_instr, history_instr_encoding)
