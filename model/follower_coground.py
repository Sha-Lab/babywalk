...
class CogroundDecoderLSTM(nn.Module):
    def __init__(self, embedding_size, hidden_size, dropout_ratio, feature_size,
                 history=False, visual_hidden_size=1024):
        super(CogroundDecoderLSTM, self).__init__()
        self.embedding_size = embedding_size
        self.feature_size = feature_size
        self.hidden_size = hidden_size
        self.u_begin = try_cuda(Variable(torch.zeros(embedding_size), requires_grad=False))
        ...
        self.positional_encoding = PositionalEncoding(hidden_size, dropout=0, max_len=100)
        ...
        self.visual_mlp = nn.Sequential(
            ...
            nn.ReLU())
        self.action_attention_layer = WhSoftDotAttention(hidden_size * 2, visual_hidden_size)
    ...
    def forward_current(self, u_t_prev, all_u_t, visual_context, h_0, c_0, ctx,
        ...
        if history_context is not None:
            ...
            action_selector = self.linear_combine(torch.cat((attn_text, h_1, context, text), 1))
        else:
